...
import no.nav.aura.basta.backend.fasit.payload.ApplicationInstancePayload;
...
public class FasitRestClient {
    ...
    public Collection<ResourcePayload> findResources(EnvClass envClass, String environment, DomainDO domain, String appName, ResourceType type, String alias) {
        URI url = buildResourceQuery(envClass, environment, domain, appName, type, alias, false, false);
        ResourcePayload[] resources = get(url, ResourcePayload[].class);
        return Arrays.asList(resources);
    }

    public ResourcePayload findBestMatchingResource(EnvClass envClass, String environment, DomainDO domain, String appName, ResourceType type, String alias) {
        URI url = buildResourceQuery(envClass, environment, domain, appName, type, alias, true, false);
        ResourcePayload[] resourceElement = get(url, ResourcePayload[].class);
    ...
    public NodePayload registerNode(NodePayload nodePayload, String comment) {
        URI uri = withComment(getBaseUrl().path("nodes"), comment).build();
        ClientRequest client = createClientRequest(uri.toString()).body(MediaType.APPLICATION_JSON, nodePayload);
        try {
            ClientResponse<NodePayload> put = client.put(NodePayload.class);
