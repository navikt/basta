    ...
    .controller('nodeListController', ['$scope', '$http', '$location', '$resource', '$rootScope', 'errorService', function ($scope, $http, $location, $resource, $rootScope, errorService) {
        ...
        $scope.ModalController = function ($scope) {
            ...
            $scope.$watch('selectedNodes', function () {
                $scope.message = 'Er du sikker på at du ønsker å dekommisjonere disse serverne: ' + _($scope.selectedNodes).map(function (n) {
                    return ' ' + n.hostname;
                });
            });
            $scope.ok = function () {
        ...
        $scope.isSelectedNode = function (node) {
            return _($scope.selectedNodes).find(function (o) {
                return node.id === o.id;
            });
        };
        $scope.setSelectedNode = function (node) {
            if (ctrlKeyDown) {
                if (!$scope.isSelectedNode(node)) {
                    ...
                    $scope.selectedNodes = _($scope.selectedNodes).filter(function (o) {
                        return node.id !== o.id;
                    });
                }
            } else {
