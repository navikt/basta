...
public class WildflyOrderRestServiceTest extends AbstractOrchestratorTest {

    private WildflyOrderRestService ordersRestService;

    @Before
    public void setup() {
        ordersRestService = new WildflyOrderRestService(orderRepository, orchestratorService);
        login("user", "user");
    }
    ...
    public void orderNewShouldGiveNiceXml() {
        ...
        Response response = ordersRestService.createWildflyNode(input.copy(), createUriInfo());
        ...
        assertRequestXML(request, "/orchestrator/request/wildfly_order.xml");
